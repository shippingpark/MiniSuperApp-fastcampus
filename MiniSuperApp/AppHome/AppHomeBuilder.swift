...
final class AppHomeBuilder: Builder<AppHomeDependency>, AppHomeBuildable { //AppHome 리불렛 객체들을 생성
  
  public override init(dependency: AppHomeDependency) { //디펜던시 인자로 받는 생성자
    super.init(dependency: dependency)
  }
  
  func build(withListener listener: AppHomeListener) -> ViewableRouting { //리스너를 인자로 받는 메서드, 리블렛에 필요한 객체들을 생성
